...
<html>
  <head>
    ...
    <center>
		...
		<hr>
		<form method="post" action="${pageContext.request.contextPath}/superadmin/searchmailbyuser.action">
		...
		<table border="1" width="1100" cellpadding="0" cellspacing="0" bordercolor="#1F85B6">
			<tr height="40" align="center">
				...
				<th width="100" style="color:#1F85B6">邮箱</th>
				<th width="100" style="color:#1F85B6">发邮件</th>
			...
			<c:forEach items="${ requestScope.upageVO.list }" var="user">
				<tr align="center">
					...
					<td>${user.nick}</td>
					<td>${user.email}</td>
					<td><form method="post"  action="${pageContext.request.contextPath}/superadmin/eshowByNo.action?&no=${user.no}"><input type="submit" value="发邮件" 
