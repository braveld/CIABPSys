...
<html>
  <head>
    ...
    <center>
		...
		<hr>
		<form method="post" action="${pageContext.request.contextPath}/superadmin/searchdanbytitle.action">
		<select id="xuan" name="xuan">
            <option value ="主题" selected = "selected">主题</option>  
		...
		<table border="1" width="1100" cellpadding="0" cellspacing="0" bordercolor="#1F85B6">
			...
			<c:forEach items="${ requestScope.upageVO.list }" var="dnote">
				<tr align="center">
					<td>${dnote.no}</td>
					<td>${dnote.title}</td>
					<td>${dnote.nick}</td>
					<td>${dnote.name}</td>
					<td>${dnote.date}</td>
					<td><form method="post"  action="${pageContext.request.contextPath}/admin/showdByNo.action?&no=${dnote.no}"><input type="submit" value="查看" 
						style="height:23px; width:60px; margin-top: 15px"></form> </td>
					<td><form method="post"  action="${pageContext.request.contextPath}/admin/deleteDan.action?&no=${dnote.no}"><input type="submit" value="删除" 
						style="height:23px; width:60px; margin-top: 15px"></form> </td>
				</tr>
