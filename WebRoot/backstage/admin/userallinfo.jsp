...
<html>
  <head>
    ...
    <center>
		...
		<hr>
		<form method="post" action="${pageContext.request.contextPath}/superadmin/searchforuli.action">
		...
		<table border="1" width="1100" cellpadding="0" cellspacing="0" bordercolor="#1F85B6">
			...
			<c:forEach items="${ requestScope.upageVO.list }" var="user">
				<tr align="center">
					...
					<td><form method="post"  action="${pageContext.request.contextPath}/admin/showByNo.action?&no=${user.no}"><input type="submit" value="查看" 
						style="height:23px; width:60px; margin-top: 15px"></form></td>
					
					<td><form method="post"  action="${pageContext.request.contextPath}/admin/deleteUser.action?&no=${user.no}"><input type="submit" value="删除" 
						style="height:23px; width:60px; margin-top: 15px"></form> </td>
				</tr>
