		...
		<table border="1" width="1100" cellpadding="0" cellspacing="0" bordercolor="#1F85B6">
			...
			<c:forEach items="${ requestScope.upageVO.list }" var="snote">
				<tr align="center">
					...
					<td>系统消息</td>
					<td>${snote.date}</td>
					<td><form method="post"  action="${pageContext.request.contextPath}/admin/showsByNo.action?&no=${snote.no}"><input type="submit" value="查看" 
						style="height:23px; width:60px; margin-top: 15px"></form> </td>
					<td><form method="post"  action="${pageContext.request.contextPath}/admin/deleteS.action?&no=${snote.no}"><input type="submit" value="删除" 
						style="height:23px; width:60px; margin-top: 15px"></form> </td>
				</tr>
